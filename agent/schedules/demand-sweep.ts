...
export function demandSweepThemeOnlyMessage(prepared: PreparedDemandSweep): string {
  ...
  return [
    ...
    "4. Call classification_audit once. Jev classified today's asks, so Jev cannot be the judge of whether they are buyer asks: you are the independent auditor and this is the one job the classifier subagent still has. If it returns `due: false`, skip to step 6. If it returns a sample, delegate ONCE to demand_ask_classifier with those quotes and ask only whether each is someone looking for a product, then call record_classification_audit with one {permalink, buyerAsk} per sampled permalink. Do not adjust your answers toward Jev's: a disagreement is the finding.",
    "5. Call build_demand_report once, with no arguments.",
    "6. Post its returned `report` field exactly as given, and nothing else. Do not summarise it, reorder it, add to it, or comment on it.",
  ].join("\n");
}
...
export function demandSweepHandoffMessage(prepared: PreparedDemandSweep): string {
  ...
  return [
    ...
    "4. Call assign_demand_themes once. Group every ask it returned in `asksToGroup` into a recurring want. That list is every ask in the window with no theme yet, so it can include asks from earlier days. Prefer an existing `themeKey` from `openThemes` whenever the want matches; open a `newLabel` only when none fits. A label names the want, not the post, for example \"LLM token spend visibility\", not \"someone asked about tokens\". An ask too vague to name a want gets its own label and will simply never recur.",
    "5. For each and only each entry in the returned `themesNeedingResearch`, delegate exactly once to the demand_viability subagent. Give it the theme label and its quotes. Set outputSchema to an object with all of: incumbentCoverage (one of \"covers\", \"partial\", \"none\"), incumbents (array of {name, covers}, at most 5, each `covers` one short clause), researchSummary (a string of at most two sentences), sources (array of {url, claim}), buildComponents (array of strings drawn ONLY from: auth, payments, third-party API integration, scraping or crawling, data pipeline or ETL, LLM feature, realtime, browser extension, mobile app, two-sided marketplace, manual ops bootstrap, regulated or compliance work). Do not research a theme outside that list and do not retry.",
